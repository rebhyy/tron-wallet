...
class StellarNetworkController extends NetworkController<IStellarAddress,
  ...
  StellarNetworkController({
    super.networks,
    required super.id,
  }) : super(type: NetworkType.stellar);
  ...
  Future<Web3StellarChainAuthenticated> createWeb3ChainAuthenticated(
    ...
    final web3Networks = _networks.values
        ...
        .toList();
    List<Web3StellarChainAccount> web3Accounts = [];
    for (final i in internalNetwork.networks) {
      ...
      final List<IStellarAddress> addresses = [];
      ...
      web3Accounts.addAll(addresses.map((e) =>
          Web3StellarChainAccount.fromChainAccount(
              address: e,
              isDefault: e == defaultAddress,
              id: e.network,
              network: network.network.coinParam.stellarChainType)));
    }
