...
base mixin MoneroChainController
  ...
  // List<MoneroSyncAccountInfo> getTrackerAddresses(
  //     MoneroSyncAccountsInfos tracker) {
  //   return tracker.indexes
  //       .map((e) => MoneroSyncAccountInfo(
  //           address: MoneroAddress(tracker.primaryAccount.account
  //               .subaddress(e.index.minor, majorIndex: e.index.major)),
  //           startHeight: e.startHeight))
  //       .toList();
  //   return _addresses.where((e) => e.addrDetails.viewKey == viewKey).toList();
  // }
  ...
  IMoneroAddress? accountFromPrimaryAddress(
    ...
    return _addresses.firstWhereOrNull((e) => e.addrDetails.viewKey == address);
  }
    ...
    await onClient(
      onConnect: (client) async {
        ...
        for (int i = 0; i < keyImages.length; i++) {
          _accountUtxos.updatePaymentStatus(utxos[i], status[i]);
        }
  ...
  void _updateAddressBalance() {
    for (final i in _addresses) {
      ...
      i._updateAddressBalance(balance);
    }
  }
  ...
  Future<void> updateChainSyncOptions({
    ...
    if (requests.isNotEmpty && chain.chain != network.coinParam.chainType) {
      throw WalletExceptionConst.internalError(
          "MoneroChainController.updateChainSyncOptions");
    }
