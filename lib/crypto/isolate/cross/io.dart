...
class _WorkerConnection {
  ...
  (CborMessageResponseArgs, int?) _getResult(WorkerMessage msg) {
    int? id;
    try {
      id = msg.id;
  ...
  static void _handleStreamCommandsToIsolate(
      ReceivePort receivePort, _IoStreamIsolateInitialData intialData) {
    receivePort.listen(intialData.sentResult);
  }
...
class _IoEncryptedIsolateInitialData {
  ...
  Future<(CborMessageResponseArgs, bool, int)> _getResult(
    ...
    try {
      ...
      final CryptoMessageArgs args =
          CryptoMessageArgs.deserialize(messageBytes);
...
class _IoStreamIsolateInitialData {
  final SendPort sendPort;
  final List<int> key;
  ...
  late final ChaCha20Poly1305 chacha = ChaCha20Poly1305(key);
  _IoStreamIsolateInitialData({required this.sendPort, required List<int> key})
  ...
  Future<(CborMessageResponseArgs, bool, int)> _getResult(
    ...
    try {
      ...
      final CryptoStreamMessageArgs args =
          CryptoStreamMessageArgs.deserialize(messageBytes);
      List<int>? encryptedPart;
      if (!encrypted) {
        final nonEncryptedMessage = msg.cast<WorkerNoneEncryptedMessage>();
        if (nonEncryptedMessage.encryptedPart != null) {
